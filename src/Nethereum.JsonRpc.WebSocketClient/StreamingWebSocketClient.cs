...
namespace Nethereum.JsonRpc.WebSocketStreamingClient
{
    ...
    public class StreamingWebSocketClient : IStreamingClient, IDisposable, IClientRequestHeaderSupport
    {
        ...
        private SemaphoreSlim _semaphoreSlimWrite = new SemaphoreSlim(1, 1);
        private SemaphoreSlim _semaphoreSlimRead = new SemaphoreSlim(1, 1);
        ...
        private async Task CloseDisposeAndClearRequestsAsync()
        {
            ...
            try
            {
                ...
                await _semaphoreSlimRead.WaitAsync(1000).ConfigureAwait(false);
                //Dispose listener
#if !NETSTANDARD1_3
                try
                {
